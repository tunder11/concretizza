...
function carregarDadosUsuario() {
  const usuarioLogado = JSON.parse(localStorage.getItem("usuarioLogado"))
  console.log("[v0] usuarioLogado:", usuarioLogado)
  if (usuarioLogado) {
    ...
    if (userRoleElement) {
      userRoleElement.textContent = usuarioLogado.role || usuarioLogado.cargo || "User"
    }

    const adminSection = document.getElementById("adminSection")
    if (adminSection) {
      console.log("[v0] cargo do usuário:", usuarioLogado.role || usuarioLogado.cargo)
      if (usuarioLogado.role === "Admin" || usuarioLogado.cargo === "Admin") {
        adminSection.style.display = "block"
        console.log("[v0] adminSection exibida")
      } else {
        adminSection.style.display = "none"
        console.log("[v0] adminSection escondida")
      }
    }
  }
}
